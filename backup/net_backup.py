...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        dl_dx = np.matmul(grad, self.params['W'].T)
        dl_dw = np.matmul(X.T, grad)
        dl_db = np.matmul(np.ones((1, len(X))), grad)

        self.gradient['W'] = dl_dw
        self.gradient['b'] = dl_db
        backward_output = dl_dx
...
class relu:
    ...
    def forward(self, X):
        ...
        self.mask = np.array(forward_output)
            ...
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    momentum[module_name+'_' +
                             key] = (_alpha*momentum[module_name + '_' + key])-_learning_rate*g
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ### gradient_update ###
            model = miniBatchStochasticGradientDescent(
                model, momentum, _lambda, _alpha, _learning_rate)
